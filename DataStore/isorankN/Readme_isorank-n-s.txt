...
Written By: Rohit Singh (rsingh@mit.edu)
            Chung-Shou Liao (csliao@ie.nthu.edu.tw)
...
######################################################
In October 2017, we provide a new version isorank-n-s-v3-64
which can speed up the original version of IsoRankN by using 
tensor products. The speed-up version, which runs for 
multiple alignment across five eukaryotic networks within 
a couple of minutes, also supports parallel computing with 
one more parameter. 
######################################################
...
./isorank  --K 30 --thresh 1e-4 --alpha 0.9 --maxveclen 1000000 data.inp
./multiway_kpartite  --K 30 --thresh 1e-4 --alpha 0.9 --maxveclen 1000000 data.inp
./isorank-n  --K 30 --thresh 1e-4 --alpha 0.9 --maxveclen 1000000 data.inp

[the speed-up version with one more parameter: threadid]
./isorank-n-s-v3-64  --K 3 --threadid 4 --thresh 1e-4 --alpha 0.6--maxveclen 1000000 data.inp 
 ...
 The next four parameters (K, thresh, maxveclen) have to do with eigenvalue
 ...
 [Note that in October 2017, we speed up IsoRankN by using tensor products.]
 ...
 --K: max number of iterations you should run this for. Really depends on how
   ...
   to go above 30 or so for large PPI networks. We recommend K=3 to 10 for IsoRankN 
   on two-way alignment. For multiple (three species or more) alignment,
   K=3 would be sufficient.
...
[The speed-up version also supports parallel computing, with one more input parameter: threadid] 
 
-- threadid: the number of cpu threads to use during execution. It depends on the machine you 
   use or how much memory space the machine has. The parameter is usually set to be 4 or 8. 
   If the machine you use can support multithread computing, of course more threads, i.e. 
   larger threadid, can more significantly reduce running time. 

   
   Here is an example with 4 threads:
   ./isorank.exe  --K 3 --threadid 4 --thresh 1e-4 --alpha 0.6--maxveclen 1000000 data.inp 
...
./isorank-n  --K 3 --threadid 4 --thresh 1e-4 --alpha 0.6 --maxveclen 1000000 --scorefile five_match-score.txt --o final_cluster.txt five.inp

Otherwise, the matching scores will be computed again while you run isorank-n.
...
isorank-n directly with star files similarly. That is,

--readstar: loading exist star files.

For example,

./isorank-n  --K 3 --threadid 4 --thresh 1e-4 --alpha 0.6 --maxveclen 1000000 --scorefile five_match-score.txt --readstar --o final_cluster.txt five.inp
...
##################################################################
In October 2017, we provide a speed-up version, isorank-n-s-v3-64
which can run for multiple alignment across five eukaryotic networks within 
a couple of minutes. 
